  ...
  <form method="post" action="<?= $isEditMode ? '/Pro_letter/documents/update_memo.php' : 'save_memo.php' ?>"
    ...
    <div class="w-[900px] mx-auto mt-16 mb-6 bg-white shadow-md rounded-md p-8" style="min-height: 1122px">
      <h1 class="text-center font-bold mb-6 text-black">
        แบบฟอร์มขอเข้าเยี่ยมศึกษาดูงาน
      </h1>
  ...
  */
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    function goSub() {
      ...
      const targetPath = selectedOption?.dataset?.url || "";
      const targetUrl = buildTemplateUrl(targetPath);

      if (!targetUrl) return;

      const nextUrl = new URL(targetUrl, window.location.origin);
      nextUrl.searchParams.set("main", String(main.value || "").trim());
      nextUrl.searchParams.set("sub", String(sub.value || "").trim());
    ...
    sub.addEventListener("change", () => {
      const selectedText = String(sub.value || "").trim();
      sub.dataset.current = selectedText;
      goSub();
    });
