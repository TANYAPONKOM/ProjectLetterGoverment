...
$signerNameValue = $formData['free_signer_name'] ?? $formData['signer_name'] ?? ($_SESSION['fullname'] ?? '');
$signerPositionValue = $formData['free_signer_position'] ?? $formData['signer_position'] ?? ($_SESSION['position'] ?? 'อาจารย์ประจำภาควิชาเทคโนโลยีสารสนเทศ');
?>
<!DOCTYPE html>
<html lang="th">
...
<body class="bg-gray-100">
  ...
  <form method="post" action="<?= hv($formAction) ?>" id="memoForm">
    ...
    <input type="hidden" name="redirect_back"
      value="/Pro_letter/form_Memo/form_memo_free_document.php?id=<?= (int)$documentId ?>">
    <?php else: ?>
    ...
    <div class="w-[900px] mx-auto mt-16 mb-8 bg-white shadow-md rounded-md p-8" style="min-height:1122px">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="flex items-center gap-3">
            ...
            <select name="sub_category" class="custom-select w-full" id="subCategory"
              data-current="<?= hv($CURRENT_SUB) ?>"
              <?= $categoryLocked ? 'data-category-locked="1" disabled' : '' ?>></select>
          ...
          <div class="flex items-center gap-3">
            ...
            <select name="faculty" class="custom-select w-full" id="faculty"
              ...
              <option value="<?= hv($facName) ?>" data-faculty-id="<?= $facId ?>"
                <?= ($facName === $currentUserFacultyName ? 'selected' : '') ?>><?= hv($facName) ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          ...
          <div class="flex items-center gap-3">
            ...
            <select name="department" class="custom-select w-full" id="dept"
              ...
              ?>
              <option value="<?= hv($deptName) ?>" data-department-id="<?= $deptId ?>"
                data-faculty-id="<?= $deptFacultyId ?>"
                <?= ($deptName === $currentUserDepartmentName || $deptId === $currentUserDepartmentId ? 'selected' : '') ?>>
                <?= hv($deptName) ?></option>
              <?php endforeach; ?>
            </select>
          </div>
      ...
      <div class="space-y-5">
        ...
        <div class="space-y-4 mb-6">
          <div class="flex items-center gap-3">
            <label class="lbl text-gray-800 whitespace-nowrap" for="signerNameInput">2.ชื่อ - นามสกุล :</label>
            <input type="text" name="free_signer_name" id="signerNameInput" class="flex-1 border rounded-md p-2"
              value="<?= hv($signerNameValue) ?>" required>
          </div>
          <div class="flex items-center gap-3">
            <label class="lbl text-gray-800 whitespace-nowrap" for="signerPositionInput">ตำแหน่ง :</label>
            <input type="text" name="free_signer_position" id="signerPositionInput" class="flex-1 border rounded-md p-2"
              value="<?= hv($signerPositionValue) ?>" required>
          </div>
        </div>

        <div class="form-row-inline">
          <label class="lbl asterisk row-label">3. เรื่อง :</label>
          <div class="w-full">
            <textarea name="subject" id="subjectInput" data-spell-field="subject" rows="2" class="field-input"
              placeholder="เช่น ขอความอนุเคราะห์ / ขออนุมัติ / ขอแจ้งเพื่อทราบ"
              required><?= hv($subjectValue) ?></textarea>
        ...
        <div class="form-row-inline">
          <label class="lbl asterisk row-label">4. เรียน :</label>
          <div class="w-full">
            <input type="text" name="to_person" id="toPersonInput" data-spell-field="to_person" class="field-input"
              placeholder="เช่น ผู้อำนวยการศูนย์พัฒนาศักยภาพบุคลากรและบริการวิชาการ" value="<?= hv($toPersonValue) ?>"
              required>
        ...
        <div class="form-row-inline">
          <label class="lbl asterisk row-label">5. เบอร์โทรภาควิชา :</label>
          <span class="text-gray-800 whitespace-nowrap">โทร.</span>
          <input type="text" name="department_phone" class="field-input max-w-[235px]"
            value="<?= hv($departmentPhoneValue) ?>" placeholder="เช่น 7064" required>
          <span class="text-gray-800 whitespace-nowrap">ที่ต้องการให้ขึ้นที่ส่วนราชการ</span>
        </div>

        <div>
          <label class="lbl asterisk block mb-1">6. เนื้อหาเอกสาร :</label>
          <textarea name="free_paragraph_1" id="paragraph1Input" data-spell-field="free_paragraph_1" rows="6"
            class="field-input" placeholder="กรอกเนื้อหาเอกสารย่อหน้าแรก"
            required><?= hv($paragraph1Value) ?></textarea>
          ...
          <div id="paragraph2Box" class="paragraph-box <?= trim((string)$paragraph2Value) !== '' ? '' : 'hidden' ?>">
            ...
            <textarea name="free_paragraph_2" id="paragraph2Input" data-spell-field="free_paragraph_2" rows="6"
              class="field-input"
              placeholder="กรอกเนื้อหาเพิ่มเติมได้ ถ้าไม่มีสามารถเว้นว่างได้"><?= hv($paragraph2Value) ?></textarea>
          ...
          <div id="paragraph3Box" class="paragraph-box <?= trim((string)$paragraph3Value) !== '' ? '' : 'hidden' ?>">
            ...
            <textarea name="free_paragraph_3" id="paragraph3Input" data-spell-field="free_paragraph_3" rows="6"
              class="field-input"
              placeholder="กรอกเนื้อหาเพิ่มเติมได้ ถ้าไม่มีสามารถเว้นว่างได้"><?= hv($paragraph3Value) ?></textarea>
  ...
  function goToSelectedForm() {
    ...
    window.location.href = '/Pro_letter' + path + '?main=' + encodeURIComponent(mainCategory.value) + '&sub=' +
      encodeURIComponent(opt.textContent.trim());
  }
