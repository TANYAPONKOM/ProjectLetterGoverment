...
<body class="bg-gray-100">
  ...
  function formatDocumentDetail(d) {
    ...
    const inviteSubject = firstDetailValue(detailSource, [
  "subject",
  "memo_subject",
  "letter_subject",
  "invite_subject",
  ...
  "title",
  "เรื่อง"
]) || cleanDetailText(d.subject || "");

if (
  hint.includes("INVITE_SPEAKER") ||
  hint.includes("infor_invite.php") ||
  hint.includes("form_memo_invite_speaker.php") ||
  hint.includes("หนังสือเรียนเชิญวิทยากร") ||
  hint.includes("เรียนเชิญวิทยากร")
) {
  return inviteSubject ? `เรื่อง: ${inviteSubject}` : "(ไม่มีรายละเอียด)";
}
