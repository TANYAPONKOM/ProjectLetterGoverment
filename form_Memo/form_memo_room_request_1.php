...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save_review_comment') {
  ...
  if ($reviewComment === '') {
    header("Location: form_memo_room_request_1.php?id=" . (int)$docId . "&comment_err=empty");
    exit;
  }
  ...
  header("Location: form_memo_room_request_1.php?id=" . (int)$docId . "&comment_saved=1");
  exit;
}
...
document.addEventListener("DOMContentLoaded", () => {
  Swal.fire({
    ...
    html: <?= json_encode(($editAlertText ?: "เอกสารนี้ไม่สามารถแก้ไขได้ในขณะนี้") . "<br><br>ระบบจะแสดงเอกสารในโหมดดูตัวอย่างเท่านั้น", JSON_UNESCAPED_UNICODE) ?>,
