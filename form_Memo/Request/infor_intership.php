...
$CURRENT_SUB  = "ระบบขอความอนุเคราะห์หนังสือฝึกงาน (ของนักศึกษา)";           // ถ้าไม่มีหมวดย่อย ให้เว้นว่าง
?>
...
<body class="bg-gray-100">
  <header class="bg-teal-500 text-white p-4 flex justify-between items-center shadow-md"
    ...
    <div class="flex items-center space-x-4">
      ...
      <a href="/Pro_letter/form_Memo/Request/infor_intership.php">
    ...
    <div class="w-[900px] mx-auto mt-16 mb-6 bg-white shadow-md rounded-md p-8" style="min-height: 1122px">
      ...
      <div class="mb-4 flex items-center gap-4">
        ...
        <input type="text" name="to_person" class="flex-1 border rounded-md p-2"
          placeholder="เช่น ผู้จัดการบริษัท / หัวหน้าหน่วยงาน">
      </div>
      ...
      <div class="mb-4 flex items-center gap-4">
        ...
        <input type="number" name="student_count" id="studentCount" class="w-32 border rounded-md p-2" min="1"
          placeholder="เช่น 3">
        <span>คน</span>
      </div>


      <!-- 3. ชื่อ–นามสกุลนักศึกษา | รหัสนักศึกษา -->
      <div class="mb-6">
        <label class="lbl block mb-2">
          3. ชื่อ–นามสกุลนักศึกษา | รหัสนักศึกษา
        </label>

        <div id="studentList" class="space-y-3 ml-6"></div>
      </div>


      <!-- 4. ช่วงเวลาที่ต้องการฝึกงาน -->
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    // 2) ลิงก์ของ “หมวดหลัก”
    // - train -> ไปหน้าฟอร์มหลักเลย
    // - academic -> ยังไม่รู้ปลายทาง ใช้ # (ไม่ redirect)
    const ROUTE_MAIN = {
    ...
    // 3) ลิงก์ของ “หมวดย่อย”
    // ยังไม่รู้หน้าไหน ใส่ # ไปก่อนตามที่คุณบอก
    const ROUTE_SUB = {
      // external
      ...
      // internal (ยังไม่รู้ลิงก์จริง ใส่ # ไปก่อน)
      "ขอเปลี่ยนแปลงตารางสอน (ของอาจารย์)": "#",
    ...
    function syncUI() {
      ...
      // ถ้าเป็น train/academic -> ไม่ต้องมีหมวดย่อย
      ...
      // ถ้าเป็น external/internal -> เปิดหมวดย่อย
      sub.disabled = false;
    ...
    function goMain() {
      ...
      // train -> redirect, academic -> # (ไม่ทำอะไร)
      if (target && target !== "#") {
    ...
    function goSub() {
      ...
      if (!target || target === "#") return; // ยังไม่รู้หน้าปลายทางก็ไม่ต้องเด้ง
      if (window.location.pathname !== target) window.location.href = target;
    }

    // events
    main.addEventListener("change", () => {
      sub.dataset.current = ""; // เปลี่ยนหมวดหลักแล้วเคลียร์ค่าเดิม
      syncUI();
      goMain();
    });
    ...
    // init: ใช้ค่า $CURRENT_MAIN / $CURRENT_SUB ที่ PHP set มา
    syncUI();
  });
  </script>
