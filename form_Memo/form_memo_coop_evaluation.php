...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save_review_comment') {
  ...
  if ($reviewComment === '') {
    header("Location: form_memo_coop_evaluation.php?id=" . (int)$docId . "&comment_err=empty");
    exit;
  }
  ...
  header("Location: form_memo_coop_evaluation.php?id=" . (int)$docId . "&comment_saved=1");
  exit;
}
...
">
        ...
        <!-- เนื้อหา -->
        <div class="thai-distribute" style="
    font-size:16pt;
    line-height:1.28;
    text-align:justify;
    text-justify:distribute;
...
">
    ...
    const addBreaks = (text) => {
      ...
      if (segmenter) {
        return Array.from(segmenter.segment(text), part => {
          ...
          return part.isWordLike ? value + '\u200B' : value;
        }).join('');
      }
      return text
        .replace(
  ...
  document.addEventListener('DOMContentLoaded', () => applyThaiDistributeWordBreaks(document));
  ...
  async function downloadPdf() {
    ...
    try {
      ...
      for (let i = 0; i < pages.length; i++) {
        ...
        applyThaiDistributeWordBreaks(clone);
