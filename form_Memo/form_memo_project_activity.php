...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save_review_comment') {
  ...
  if ($reviewComment === '') {
    header("Location: form_memo_project_activity.php?id=" . (int)$docId . "&comment_err=empty");
    exit;
  }
  ...
  header("Location: form_memo_project_activity.php?id=" . (int)$docId . "&comment_saved=1");
  exit;
}
