...
", $reviewComment);
  ...
  if ($reviewComment === '') {
    header("Location: form_consent_research_presentation.php?id=" . (int)$docId . "&comment_err=empty");
    exit;
  }
  ...
  header("Location: form_consent_research_presentation.php?id=" . (int)$docId . "&comment_saved=1");
  exit;
}
  ...
  <main class="page">
    <form id="updateForm" action="update_memo.php" method="post">
      ...
      <div class="footer-actions">
        ...
        <!-- ปุ่มแก้ไขเอกสาร -->
        <?php if ($canEdit): ?>
        <a href="/Pro_letter/documents/infor_present.php?id=<?= (int)$docId ?>&edit=1"
