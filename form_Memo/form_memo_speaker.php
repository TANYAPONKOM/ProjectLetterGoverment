...
$refDate    = $valueMap[20] ?? "";   // วันที่อ้างอิง (ต้องดูว่า field_id จริงใน DB คืออะไร)
$eventRange = $valueMap[21] ?? "";   // ช่วงวันที่จัดงาน
$eventPlace = $valueMap[22] ?? "";   // สถานที่จัดงาน
...
">
      ...
      <div class="doc-row subject-row" style="margin-bottom:25px;">
        ...
        <div class="dot-line subject-line">
          <span class="chip" contenteditable="true" style="display:inline-block; padding-left:28px;">
            ขออนุมัติคัตตัวบุคคลเป็นวิทยากรบรรยายในโครงการอบรมเชิงปฏิบัติการ
          </span>
        </div>
      </div>
  ...
  async function downloadPdf() {
    try {
      ...
      const cloneActions = clone.querySelector(".footer-actions");
      ...
      const garuda = clone.querySelector('img[src*="g_photo1"]');
      if (garuda) {
